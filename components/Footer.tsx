    ...
    <footer className="bg-gray-900 text-gray-300">
      ...
      <div className="bg-green-600">
        <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="text-white font-bold text-xl">Ready to make your home safer?</p>
            <p className="text-green-50 text-sm">Free inspection · Same day installation · Best prices guaranteed</p>
          </div>
          <div className="flex gap-3">
            <a href={BUSINESS.whatsapp} target="_blank" rel="noopener noreferrer"
              aria-label="Chat with us on WhatsApp"
              className="flex items-center gap-2 px-5 py-3 bg-white text-green-700 font-bold rounded-xl hover:bg-green-50 transition-colors shadow-lg text-sm">
            ...
            <a href={BUSINESS.phoneHref}
              ...
              className="flex items-center gap-2 px-5 py-3 bg-green-800 text-white font-bold rounded-xl hover:bg-green-900 transition-colors text-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {/* Brand */}
        <div>
          <h3 className="text-white font-bold text-lg mb-3">{BUSINESS.name}</h3>
          <p className="text-gray-400 text-sm leading-relaxed mb-4">{BUSINESS.description}</p>
          <div className="space-y-2 text-sm">
            <a href={BUSINESS.phoneHref} aria-label={`Call ${BUSINESS.phoneDisplay}`} className="flex items-center gap-2 text-green-300 hover:text-green-200 transition-colors">
            ...
            <a href={BUSINESS.emailHref} aria-label={`Email: ${BUSINESS.email}`} className="flex items-center gap-2 text-gray-300 hover:text-white transition-colors break-all">
            ...
            <p className="flex items-center gap-2 text-gray-400" aria-label="Location: Bangalore, Karnataka, India">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {SERVICES.map((s) => (
              <li key={s.slug}>
                <Link href={`/${s.slug}`} className="text-gray-400 hover:text-green-400 text-sm transition-colors">{s.title}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {LOCATIONS.map((l) => (
              <li key={l.slug}>
                <Link href={`/${l.slug}`} className="text-gray-400 hover:text-green-400 text-sm transition-colors">{l.area}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <div className="mt-6 p-3 bg-gray-800 rounded-xl text-center">
            <p className="text-yellow-400 font-bold text-lg">⭐ {BUSINESS.rating}/5</p>
            <p className="text-gray-400 text-xs">{BUSINESS.reviewCount}+ Google Reviews</p>
          </div>
        </div>
      </div>
